...
router.get('/status', async (req, res) => {
    try{
        ...
        res.status(200).json({ status: 'OK', message: 'Backend ve DB bağlantisi başarili'});
    }catch (err){
        console.error('DB Bağlantisi Hatasi:', err);
        res.status(503).json({status: 'ERROR', message: 'Veritabanina bağlanilamadi'});
    }
});
